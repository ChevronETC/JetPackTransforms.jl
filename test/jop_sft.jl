...
@testset "JopSft" begin

    @testset "Sft dot product test, T=$(T)" for T in (Float64,Float32)
        global n1,n2,dt
        f = fftfreq(n1) ./ dt
    ...
    @testset "Sft linearop is linear, T=$(T)" for T in (Float64,Float32)
        global n1,n2,dt
        f = fftfreq(n1) ./ dt
        freqs = T[f[5],f[9],f[13]]
        nfreq = length(freqs)
    ...
    @testset "Sft correctness test, T=$(T)" for T in (Float64,)
        global n1,n2,dt
        f = fftfreq(n1) ./ dt
        freqs = T[f[5],f[9],f[13]]
        nfreq = length(freqs)
